...
const SOURCE_HASH = 'sha256:2acecd7be91125f191f2039da3905eac0e54b19db32ea3e87d7a9c3acca4df61';
...
(async () => {
  ...
  const publicError = await page.locator('#survey-question-question-1').getAttribute('aria-invalid');
  ...
  const publicOverflow = await page.evaluate(() => ({ scrollWidth: document.documentElement.scrollWidth, innerWidth: window.innerWidth, fits: document.documentElement.scrollWidth <= window.innerWidth }));
  assert(publicError === 'true', 'required survey error was not rendered');
  assert(await page.getByRole('radio', { name: 'Attend' }).isVisible(), 'English choice not visible');
  assert(!/phone|hash|ciphertext|reviewer/i.test(publicBody), 'private text leaked on public survey');
  assert(publicOverflow.fits, 'public page overflowed at 320px');

  await page.getByLabel('Language').selectOption('ko');
  await page.getByRole('heading', { name: '공개 설문' }).waitFor();
  await page.goto('http://127.0.0.1:4173/admin/surveys/survey-1/responses');
  await page.getByRole('button', { name: '열기', exact: true }).click();
  await page.getByRole('alert').waitFor();
  const staleActionAbsent = await page.getByRole('button', { name: '반려' }).count() === 0;
  assert(staleActionAbsent, 'stale response detail remained actionable');
  await page.getByRole('button', { name: '다시 시도' }).click();
  await page.getByText('참여하시겠습니까?').waitFor();
  await page.getByRole('textbox', { name: '거절 사유' }).fill('No capacity');
  await page.getByRole('button', { name: '반려' }).click();
  const cancelFocused = await page.getByRole('dialog').getByRole('button', { name: '취소' }).evaluate((el) => el === document.activeElement);
  await page.getByRole('dialog').getByRole('button', { name: '취소' }).click();
  await page.getByRole('button', { name: '반려' }).click();
  await page.getByRole('dialog').getByRole('button', { name: '확인' }).click();
  await page.waitForFunction(() => window.__surveyQaReviewBody !== undefined, null, { timeout: 100 }).catch(() => {});
  assert(JSON.stringify(reviewBody) === JSON.stringify({ expectedSurveyRevisionId: 'revision-1', state: 'REJECTED', reason: 'No capacity' }), 'review payload did not preserve response tuple');
  ...
  assert(dialogText.includes('PUBLIC') && dialogText.includes('N1'), 'PUBLIC materialization disclosure missing');
  const adminOverflow = await page.evaluate(() => ({ scrollWidth: document.documentElement.scrollWidth, innerWidth: window.innerWidth, fits: document.documentElement.scrollWidth <= window.innerWidth }));
  assert(adminOverflow.fits, 'admin page overflowed at 320px');
  await page.screenshot({ path: 'qa-artifacts/survey-gen3-admin-public-confirmation-320.png', fullPage: true });
  await page.getByRole('dialog').getByRole('button', { name: '취소' }).click();
  await page.getByRole('button', { name: '공개 행사 만들기' }).click();
  await page.getByRole('dialog').getByRole('button', { name: '확인' }).click();
  assert(JSON.stringify(materializeBody) === JSON.stringify({ location: 'N1', visibility: 'PUBLIC' }), 'PUBLIC materialization payload mismatch');
  assert(await page.getByRole('link', { name: '행사 열기' }).getAttribute('href') === '/events/event-1', 'event link mismatch');
  const adminBody = await page.locator('body').innerText();
  assert(!/phone|hash|ciphertext|reviewer/i.test(adminBody), 'private text leaked in admin flow');
  const transcript = { sourceHash: SOURCE_HASH, status: 'passed', scenario: 'generation-3 live Chromium mocked survey public/admin adversarial flow', viewport: { width: 320, height: 700 }, assertions: { publicRequiredError: publicError === 'true', localeEnglish: true, publicPrivacyNonDisclosure: !/phone|hash|ciphertext|reviewer/i.test(publicBody), publicActionGeometry: publicOverflow, staleDetailNonActionable: staleActionAbsent, reviewCancelFocus: cancelFocused, exactRejectedReviewPayload: true, publicConfirmationDisclosure: dialogText.includes('PUBLIC') && dialogText.includes('N1'), adminActionGeometry: adminOverflow, exactPublicMaterializationPayload: true, eventLink: true, adminPrivacyNonDisclosure: !/phone|hash|ciphertext|reviewer/i.test(adminBody) }, requests };
  fs.writeFileSync('qa-artifacts/survey-gen3-browser-transcript.json', `${JSON.stringify(transcript, null, 2)}\n`);
  await browser.close();
