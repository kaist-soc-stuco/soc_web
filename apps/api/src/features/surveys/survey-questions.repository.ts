...
export class SurveyQuestionsRepository {
  ...
  private map(row: typeof surveyQuestions.$inferSelect): SurveyQuestionRecord {
    return {
      ...
      config: row.config as SurveyQuestionConfig | null,
  ...
  async reorder(
    sectionId: string,
    items: ReorderSurveyQuestionsRequest["items"],
    tx?: PostgresTransaction,
  ): Promise<SurveyQuestionRecord[]> {
    const db = tx ?? this.db;
    ...
    for (const item of items) {
      await db
        ...
        .where(and(eq(surveyQuestions.id, item.id), eq(surveyQuestions.sectionId, sectionId)));
    }
    return this.findBySectionId(sectionId, tx);
  }
}
