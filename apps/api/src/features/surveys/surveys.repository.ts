...
export class SurveysRepository {
  ...
  async replaceSections(id: string, actor: string, input: Array<{ ordinal: number; title: { kr: string; en: string }; description?: { kr: string; en: string } | null }>, correlationId: string) { return this.db.transaction(async (tx) => { const revision = await this.lockDraft(tx, id); if (!revision) return (await this.exists(tx, id)) ? 'IMMUTABLE' as const : 'MISSING' as const; await tx.delete(surveySections).where(eq(surveySections.surveyRevisionId, revision.id)); if (input.length) await tx.insert(surveySections).values(input.map((s) => ({ surveyRevisionId: revision.id, ordinal: s.ordinal, titleKr: s.title.kr, titleEn: s.title.en, descriptionKr: s.description?.kr ?? null, descriptionEn: s.description?.en ?? null }))); await this.audit(tx, id, null, actor, 'SURVEY_SECTIONS_REPLACED', 'sections', correlationId); return 'UPDATED' as const; }); }
