    ...
    <AuthGuard requirePermission={Permissions.MANAGE_VOTE}>
      <AdminPageShell><AdminPageMain className="admin-vote-editor">
        ...
        {vote && vote.status !== "DRAFT" ? <AdminCard>
          ...
          <div className={`grid gap-3 border-b border-slate-100 p-4 sm:p-5 ${rosterEditable ? "md:grid-cols-2" : ""}`}><UiInput placeholder="명부 검색" value={voterQuery} onChange={(e) => setVoterQuery(e.target.value)} />{rosterEditable ? <div className="flex min-w-0 flex-wrap gap-2"><UiInput className="min-w-0 flex-1" placeholder="추가할 이름·학번 검색" value={candidateQuery} onChange={(e) => setCandidateQuery(e.target.value)} /><Button variant="outline" onClick={() => void searchCandidates()}><UserPlus />검색</Button></div> : null}</div>
