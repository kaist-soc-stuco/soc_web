...
export function AdminBoardsPage() {
    ...
    if (grants.status !== 'ready')
        return <section><h1 className="text-[32px] font-extrabold">{uiText("pages.admin-boards-page.887f65e5fe")}</h1><p className="mt-5 text-sm font-semibold">{uiText("pages.admin-boards-page.fd041853ed")}</p></section>;
    if (!allowed)
        return <section><h1 className="text-[32px] font-extrabold">{uiText("pages.admin-boards-page.887f65e5fe")}</h1><p role="alert" className="mt-5 text-sm font-semibold text-red-700">{uiText("pages.admin-boards-page.597c4dcbcb")}</p></section>;
    if (state === 'denied')
        return <section><h1 className="text-[32px] font-extrabold">{uiText("pages.admin-boards-page.887f65e5fe")}</h1><p role="alert" className="mt-5 text-sm font-semibold text-red-700">{uiText("pages.admin-boards-page.338cc43b50")}</p><button type="button" onClick={() => void refresh()} className="mt-4 rounded-[5px] border border-kaist-grey/25 px-4 py-2 text-xs font-extrabold">{uiText("pages.admin-boards-page.bf19890318")}</button></section>;
    return <section>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-kaist-grey/25 pb-4"><h1 className="text-[32px] font-extrabold tracking-tight">{uiText("pages.admin-boards-page.887f65e5fe")}</h1><button type="button" onClick={() => void refresh()} disabled={state === 'loading' || pending} className="rounded-[5px] border border-kaist-grey/25 px-4 py-2 text-xs font-extrabold">{uiText("pages.admin-boards-page.bf19890318")}</button></div>
