...
export function AdminFaqsPage() {
  ...
  useEffect(() => { void load(); }, []);
  const act = async (operation: () => Promise<unknown>, success: string) => { setMessage(''); try { await operation(); await load(); setMessage(success); } catch { setMessage('요청을 처리하지 못했습니다.'); } };
  ...
  return <section>
    ...
    <form onSubmit={createTopic} className="mt-6 rounded-lg border bg-white p-5">
      ...
      <button className="rounded bg-kaist-darkgreen px-4 py-2 font-bold text-white">주제 추가</button></div>
    </form>
    <form onSubmit={createFaq} className="mt-6 rounded-lg border bg-white p-5">
      ...
      <button disabled={!faq.topicId} className="rounded bg-kaist-darkgreen px-4 py-2 font-bold text-white disabled:opacity-50">FAQ 추가</button></div>
    </form>
