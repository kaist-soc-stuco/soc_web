...
import type { EventItem, EventVisibility } from '@soc/contracts';
...
export function AdminEventsPage() {
  ...
  const load = async () => { try { setItems((await adminEventApi.list()).items); } catch { setMessage('행사 정보를 불러오지 못했습니다.'); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
  const submit = async (event: FormEvent) => {
    ...
    try {
      await adminEventApi.create({ titleKr: form.titleKr, titleEn: form.titleEn, descriptionKr: form.descriptionKr, descriptionEn: form.descriptionEn, startAtMs: new Date(form.start).getTime(), endAtMs: new Date(form.end).getTime(), allDay: false, location: form.location, visibility: form.visibility });
      setForm(initial); await load(); setMessage('행사를 추가했습니다.');
