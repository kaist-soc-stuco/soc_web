...
export function AdminSurveyEditPage() {
  ...
  const updateItems = (sectionIndex: number, mutate: (items: SurveySectionItemDefinitionInput[]) => SurveySectionItemDefinitionInput[]) => {
    if (!survey || !english || !editable) return;
  ...
  const saveDefinition = async () => {
    if (!survey || !surveyId || !editable) return; setSaving(true); setError('');
  ...
  const addSection = () => { if (!survey || !english || !editable) return; const ordinal = survey.sections.length; const section = { id: localId(), ordinal, title: { value: '', translationUnavailable: false }, items: [] }; setSurvey({ ...survey, sections: [...survey.sections, section] }); setEnglish({ ...english, sections: [...english.sections, section] }); setDefinitionDirty(true); };
...
function Insertion({ editable, onAdd }: { editable: boolean; onAdd: (kind: 'DESCRIPTION' | 'IMAGE_BLOCK' | 'QUESTION') => void }) { return <div className="my-2"><button type="button" disabled={!editable} aria-label="항목 추가" onClick={() => onAdd('QUESTION')}>+</button><button type="button" disabled={!editable} onClick={() => onAdd('DESCRIPTION')}>설명 추가</button><button type="button" disabled={!editable} onClick={() => onAdd('IMAGE_BLOCK')}>이미지 추가</button></div>; }
