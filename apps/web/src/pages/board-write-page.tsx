...
export function BoardWritePage() {
  ...
  useEffect(() => {
    ...
    const preferredCategory =
      categoryParam && writableBoardCodes.includes(categoryParam)
        ? categoryParam
  ...
  const handleCategoryChange = (nextCategory: string) => {
    ...
    navigate(`/board/${nextCategory}/write`, { replace: true });
  };
  ...
  }, [titleKo, contentKo, titleEn, contentEn, isAnonymous, isPinned, isKoreanOnly, eventStartDate, eventEndDate, eventDescription, selectedCategory]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const article = await apiClient.createArticle(selectedCategory, {
        ...
        eventStartDate: selectedCategory === "행사" ? htmlDatetimeLocalToIso(eventStartDate) : undefined,
        eventEndDate: selectedCategory === "행사" ? htmlDatetimeLocalToIso(eventEndDate) : undefined,
        eventDescription: selectedCategory === "행사" ? eventDescription.trim() : undefined,
      });
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex flex-col text-slate-950">
      ...
      <PageHero
        ...
        description={lang === "ko" ? "새로운 소식을 국문과 영문으로 공유해보세요." : "Share news in Korean and English."}
      />
            ...
            <div className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[12px] font-bold text-slate-500">
                  {lang === "ko" ? "게시판 선택" : "Board category"}
                </p>
                <p className="mt-0.5 text-[11px] font-medium text-slate-400">
                  {lang === "ko"
                    ? "작성 권한이 있는 게시판만 표시됩니다."
                    : "Only boards you can write to are shown."}
                </p>
              </div>
              <select
                ...
                className="h-9 min-w-[180px] rounded-lg border border-slate-200 bg-white px-3 text-[13px] font-bold text-slate-800 shadow-sm outline-none transition focus:border-kaist-darkgreen focus:ring-2 focus:ring-kaist-darkgreen/10 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
              >
                {writableBoardCodes.length > 0 ? (
                  ...
                  <option value={selectedCategory}>
                    {lang === "ko" ? "작성 가능한 게시판 없음" : "No writable board"}
                  </option>
                )}
              </select>
            </div>
