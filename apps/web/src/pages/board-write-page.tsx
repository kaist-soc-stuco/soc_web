...
export function BoardWritePage() {
  ...
  useEffect(() => {
    ...
    boardApi.get(category, 'ko', controller.signal)
      ...
      .catch((cause: unknown) => {
        if ((cause as { name?: string }).name !== 'AbortError') setError('게시판 정보를 불러오지 못했습니다.');
      })
    ...
    <SiteLayout>
      <div className="min-h-[calc(100vh-4.5rem)] bg-[#F7FCFC]">
        ...
        <section className={`${pageContainerClass} pb-16 py-2`}>
          ...
          {loading ? <p className="py-12 text-center text-kaist-grey">게시판 정보를 불러오는 중입니다.</p> : <form onSubmit={submit}>
