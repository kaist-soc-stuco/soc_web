...
export function BoardPage() {
    ...
    const handlePageChange = async (page: number) => {
        if (page <= 0 || page === currentPage || page > currentPage + 1)
            return;
            ...
            <div className={`${pageContainerClass} flex flex-wrap items-end justify-between gap-8`}>
              ...
              <div className="flex items-center">
                <div className="relative flex items-center gap-2 border-b border-kaist-darkgreen/40 mb-2.5">
                  <span className="text-base font-semibold text-[#9AA69F]">{uiText("pages.board-page.078b3a1b0a")}</span>
          ...
          <div className="flex gap-6">
            <div className="flex-[5]">
              ...
              <div className="mt-8 flex items-center justify-center relative">
                {(currentPage > 1 || nextCursor) && (<div className="flex items-center gap-2">
                ...
                {canCreate && (<Link to={`/board/${category}/write`} className="absolute right-0 rounded-[5px] border border-kaist-darkgreen bg-white px-4 py-2 text-xs font-semibold tracking-tight text-kaist-darkgreen transition-colors hover:bg-kaist-darkgreen hover:text-kaist-white">{uiText("pages.board-page.b22f31b432")}</Link>)}
              </div>
            </div>
          </div>
        </div>
