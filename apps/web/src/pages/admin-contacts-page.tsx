...
export function AdminContactsPage() {
  ...
  const load = (mode = projection, cursor?: string) => {
    ...
    contactApi.list(mode, undefined, cursor).then((page) => {
      ...
      setStatus('ready');
    }).catch((error: unknown) => { setStatus('error'); setMessage(error instanceof ContactApiError && error.status === 403 ? '연락처 관리 권한이 없습니다.' : '연락처 정보를 불러오지 못했습니다.'); });
  };
  ...
  const remove = async () => {
    ...
    setSubmitting(true);
    try { await contactApi.remove(pendingDelete.id, { reasonCode: 'ADMIN_REQUEST' }); setContacts((current) => current.filter((contact) => contact.id !== pendingDelete.id)); setPendingDelete(null); }
    catch { setMessage('연락처를 삭제하지 못했습니다.'); }
    finally { setSubmitting(false); }
  };
  return <section>
    ...
    <form onSubmit={submit} className="mb-6 grid gap-3 rounded-[8px] border border-kaist-grey/25 bg-white p-4 md:grid-cols-2">
      ...
      <div className="flex items-end gap-2"><button type="submit" disabled={submitting} className="rounded-[5px] bg-kaist-darkgreen px-5 py-2 text-xs font-extrabold text-white disabled:opacity-50">{submitting ? '저장 중...' : editing ? '수정 저장' : '연락처 추가'}</button>{editing && <button type="button" onClick={() => { setEditing(null); setForm(emptyForm()); }} disabled={submitting} className="rounded-[5px] border border-kaist-grey/25 px-5 py-2 text-xs font-extrabold">취소</button>}</div>
    </form>
    ...
    <div className="overflow-x-auto"><div className="grid min-w-[1120px] grid-cols-[1fr_1fr_1.2fr_1.5fr_1.5fr_1.5fr_0.8fr] items-center border-b-2 border-kaist-darkgreen-main py-3 text-sm font-extrabold tracking-tight text-kaist-darkgreen"><div>이름</div><div>직책</div><div>이메일</div><div>전화번호</div><div>직장</div><div>비고</div><div>관리</div></div><div className="min-w-[1120px] divide-y divide-kaist-grey/20 border-b border-kaist-grey/20">
      ...
      {status === 'ready' && contacts.map((contact) => <div key={contact.id} className="grid grid-cols-[1fr_1fr_1.2fr_1.5fr_1.5fr_1.5fr_0.8fr] items-center gap-2 py-3 text-sm"><div>{contact.name}</div><div>{contact.role ?? '-'}</div><div>{contact.email ?? '-'}</div><div>{contact.phone ?? '-'}</div><div>{contact.affiliation ?? '-'}</div><div>{contact.note ?? '-'}</div><div className="flex gap-2"><button type="button" aria-label={`${contact.name} 수정`} onClick={() => { setEditing(contact.id); setForm(contactForm(contact)); }} className="text-kaist-darkgreen">수정</button><button type="button" aria-label={`${contact.name} 삭제`} onClick={() => setPendingDelete(contact)} disabled={submitting} className="text-red-700">삭제</button></div></div>)}
    </div></div>
    {nextCursor && status === 'ready' && <button type="button" onClick={() => load(projection, nextCursor)} className="mt-4 rounded border px-4 py-2 font-bold">더 보기</button>}
    {pendingDelete && <div role="dialog" aria-modal="true" aria-labelledby="contact-delete-title" className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4"><div className="rounded bg-white p-6"><h2 id="contact-delete-title" className="text-lg font-extrabold">{pendingDelete.name} 연락처 삭제</h2><p className="mt-2">이 작업은 되돌릴 수 없습니다.</p><div className="mt-5 flex justify-end gap-3"><button type="button" disabled={submitting} onClick={() => setPendingDelete(null)}>취소</button><button type="button" disabled={submitting} onClick={() => void remove()} className="rounded bg-red-700 px-4 py-2 font-bold text-white">{submitting ? '삭제 중...' : `${pendingDelete.name} 삭제 확인`}</button></div></div></div>}
  </section>;
}
