import type { AdminContactListResponse, AppErrorResponse, ChatPageResponse, ChatMessageRequest, ContactDto, CreateContactRequest, DeleteContactRequest, MailCancelRequest, MailCreateRequest, MailGetRequest, MailPreviewRequest, PatchContactRequest } from '@soc/contracts';
...
const isChatPage = (value: unknown): value is ChatPageResponse => exact(value, ['kind', 'externalUrl', 'notice']) && value.kind === 'EXTERNAL_LINK_NOTICE' && string(value.externalUrl) && string(value.notice);
...
export const contactApi = {
  ...
  mailPreview: (input: MailPreviewRequest) => request('/admin/mail/preview', 'POST', input).then((value) => decode(value, isDisabled)),
  mailCreate: (input: MailCreateRequest) => request('/admin/mail', 'POST', input).then((value) => decode(value, isDisabled)),
  ...
  chatMessage: (input: ChatMessageRequest) => request('/chat/messages', 'POST', input).then((value) => decode(value, isDisabled)),
};
