...
export function Calendar({ clickable = false }: CalendarProps) {
    ...
    const [viewMs, setViewMs] = useState<number>(() => timeObjToMs({ ...todayTime, day: 1, hour: 0, minute: 0, second: 0, millisecond: 0 }));
    ...
    const generateCalendarDays = () => {
        ...
        for (let day = 1; day <= lastDayTime.day; day++) {
            const isToday = currentYear === todayTime.year &&
                currentMonth === todayTime.month &&
                day === todayTime.day;
            // TODO: MySQL에서 이벤트 데이터 가져오기
            let event = null;
            if (day === 2) {
                event = { title: uiText("components.organisms.calendar.6db3e216c1"), color: 'bg-kaist-darkgreen' };
            }
            else if (day === 17) {
                event = { title: uiText("components.organisms.calendar.f50cb40b64"), color: 'bg-kaist-brown' };
            }
    ...
    return (<section className={`h-full bg-kaist-white flex flex-col ${clickable ? 'cursor-pointer' : ''}`} onClick={clickable ? () => navigate('/calendar') : undefined}>
