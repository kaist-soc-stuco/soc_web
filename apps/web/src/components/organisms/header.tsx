...
export function Header({ showLogo = false }: HeaderProps) {
  ...
  // 세션 정보를 기반으로 유저 정보 계산 (무조건 영어 이름 우선 노출)
    ...
    >
      <div className="flex h-14 w-full items-stretch justify-between">
        ...
        <div className="flex items-stretch">
          ...
          {/* Spacer to align nav exactly 48px to the right when showLogo is false, giving correct offset calculation */}
        ...
        {/* Right Section: Search, Notification, Language Switcher, Login/User */}
        <div className="flex items-center gap-2 md:gap-4 pr-6 relative">
          ...
          {user ? (
            <div className="relative">
              <button
                className="flex items-center gap-2 px-3 py-1 rounded hover:bg-kaist-darkgreen/5 font-bold text-kaist-black cursor-pointer"
      ...
      <div
        className={`absolute left-0 w-full bg-white shadow-[0_15px_30px_rgba(0,0,0,0.06)] border-t border-slate-50 overflow-hidden transition-all duration-300 ease-out ${hoveredIndex !== null
                    ...
                    >
                      <Link
                        to={
                          item.isBoard
                            ? `/board/${subItem}`
                            : item.label === "행사 / 설문" || item.label === "Events / Surveys"
                            ? [
                                "/events-surveys?tab=event",
                                "/events-surveys?tab=survey",
                                "/events-surveys?tab=calendar",
                              ][subIndex]
                            : item.label === "소개 (About)" || item.label === "About"
