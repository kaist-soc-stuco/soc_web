...
export function RelatedContentCards({ subject, locale }: {
    ...
    <ul className="mt-3 grid gap-3 md:grid-cols-2">{items.map((item) => <li key={`${item.kind}-${item.id}`}><Link to={item.href} className="block rounded border bg-white p-4 shadow-sm"><span className="text-xs font-bold text-kaist-darkgreen">{item.kind === 'ARTICLE' ? uiText("components.organisms.related-content-cards.847c8582b6") : item.kind === 'EVENT' ? uiText("components.organisms.related-content-cards.a6e55f8c8f") : uiText("components.organisms.related-content-cards.641da67d79")}</span><strong className="mt-1 block">{item.title}</strong>{item.kind === 'EVENT' && <time className="mt-1 block text-sm text-slate-600">{formatScheduleDate(item.startsAt)}</time>}{item.kind === 'SURVEY' && item.closesAt && <time className="mt-1 block text-sm text-slate-600">{formatScheduleDate(item.closesAt)}{uiText("components.organisms.related-content-cards.bf9a528f2b")}</time>}</Link></li>)}</ul>
  </section>;
}
