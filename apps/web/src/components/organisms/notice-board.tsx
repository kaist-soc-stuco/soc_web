    ...
    return (<section className="flex h-full min-h-0 flex-col bg-kaist-white lg:pr-9" aria-label={uiText("components.organisms.notice-board.4303086e18")}>
      {status === 'ready' && boards.length > 0 ? (<>
          ...
          {selected && (<div className="flex flex-1 flex-col py-4">
              ...
              <Link to={`/board/${encodeURIComponent(selected.code)}`} className="mt-auto self-end rounded-[5px] bg-kaist-darkgreen px-5 py-2 text-sm font-extrabold text-white">{uiText("components.organisms.notice-board.4748261af6")}</Link>
            </div>)}
