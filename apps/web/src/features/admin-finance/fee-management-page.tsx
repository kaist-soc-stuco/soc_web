...
import { FeeStatisticsPanel } from "./fee-statistics-panel";
...
const formatSemesterLabel = (value: string) => {
  ...
  return /^\d{4}$/.test(year ?? "") && (term === "1" || term === "2")
    ? `${year}학년도 ${term}학기`
    : value;
};
...
export function FeeManagementPage() {
  ...
  const detailSummary = useMemo(() => {
    ...
    return {
      ...
      latestStartSemester: detail.history[0]?.effectiveStartSemester ?? detail.status.coverageStartSemester,
      latestCoverageSemesters: detail.history[0]?.coverageSemesters ?? detail.status.coverageSemesters,
  ...
  const downloadTemplate = () => {
    ...
    const data = XLSX.utils.aoa_to_sheet([["학번", "상태", "납부 금액", "납부 적용 학기 수", "비고"]]);
    data["!cols"] = [{ wch: 18 }, { wch: 14 }, { wch: 16 }, { wch: 16 }, { wch: 40 }];
    XLSX.utils.book_append_sheet(workbook, data, "납부 데이터");
    const guide = XLSX.utils.aoa_to_sheet([["작성 안내"], ["학번은 텍스트로 입력하세요. 이름만으로 매칭하지 않습니다."], ["상태: PAID / PARTIAL / UNPAID, 납부 금액: 0 이상의 정수, 납부 적용 학기 수: 1~6"], ["현재 요약 상태 정정용입니다. 신규 수납은 납부 처리에서 원장에 기록하세요."], ["다음은 합성 예시입니다. 실제 학번으로 바꾸고 데이터 시트에 입력하세요."], ["20990001", "PAID", 45000, 6, "합성 예시"]]);
    guide["!cols"] = [{ wch: 90 }];
  ...
  const submitDetailPayment = async () => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(detailPaymentDate)) {
      setOperationError("납부 일자를 입력하세요.");
      return;
    }
    ...
    try {
      setSaving(true);
      setOperationError(null);
      const requestFingerprint = JSON.stringify([payment]);
      if (detailPaymentIdempotencyRef.current?.fingerprint !== requestFingerprint) {
        detailPaymentIdempotencyRef.current = { fingerprint: requestFingerprint, key: randomId() };
      }
      await apiClient.processStudentFeePayments({
...
function parseFeeSpreadsheet(input: ArrayBuffer): { updates: BulkUpdateStudentFeeStatusRequest["updates"]; errors: string[] } {
  ...
  const indexes = { userId: indexOf("userid", "사용자id"), stdNo: indexOf("stdno", "학번"), status: indexOf("status", "상태", "납부여부"), paidAmount: indexOf("paidamount", "납부금액", "실납부액", "수납액", "금액"), coverageSemesters: indexOf("coveragesemesters", "적용학기", "적용학기수", "납부적용학기수"), note: indexOf("note", "비고", "메모") };
  ...
  rows.slice(1).forEach((row, lineIndex) => {
    ...
    if (coverageSemesters !== undefined && (!Number.isInteger(coverageSemesters) || coverageSemesters < 1 || coverageSemesters > 6)) errors.push(`${rowLabel}: 납부 적용 학기 수는 1～6 사이여야 합니다.`);
