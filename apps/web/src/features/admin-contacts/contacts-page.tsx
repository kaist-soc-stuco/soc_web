...
import { AdminEmptyState, AdminFormField, AdminLoadingState, AdminPageHeader, AdminPageShell, AdminTableCard, AdminTableViewport } from "@/components/ui/admin-page";
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {departments.map((department) => <span key={department.id} className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 py-1 pl-3 pr-1 text-sm">
                ...
                <button type="button" aria-label={`${department.nameKo} 삭제`} data-tooltip={`${department.nameKo} 삭제`} onClick={() => void handleDepartmentDelete(department)} className="inline-flex size-6 items-center justify-center rounded-full text-slate-400 hover:bg-slate-200 hover:text-rose-600"><X className="size-3.5" /></button>
              </span>)}
            </div>
