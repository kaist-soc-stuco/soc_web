...
export function useBoardEditPageController(forcedCategory?: string) {
  ...
  const canConfigurePostSettings = Boolean(board);
  ...
  useEffect(() => {
    apiClient
      ...
      .then((response) => {
        setBoard(response);
        setAllowSecret(response.allowSecret);
      })
      .catch(() => {
        setBoard(null);
        setAllowSecret(false);
      });
  }, [apiClient, category]);

  useEffect(() => {
    if (!canUseOfficialIdentity) setIsOfficial(false);
