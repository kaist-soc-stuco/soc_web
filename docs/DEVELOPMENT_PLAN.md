...
현재 구현 상태: KAIST SSO 사용자는 학과와 무관하게 로그인할 수 있고, 비활성 계정은 세션을 revoke한 뒤 접근을 차단한다. 전산학부 주전공 등 참여 자격은 설문·투표별 정책에서 검증한다. 댓글·대댓글 알림 저장, unread count, 단건/전체 읽음 처리는 구현했다. 좋아요·스크랩·행사·관리자 공지 알림과 사용자별 알림 설정은 후속 확장으로 남아 있다.
